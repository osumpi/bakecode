...
class MqttRuntime {
  ...
  /// Initializes MQTT Runtime by providing `config`.
  ///
  /// The `config` map should contain the configuration details as in example:
  /// ```dart
  /// var config = {
  ///   'server' : 'localhost',
  ///   'port' : 1883,
  ///   'username' : 'bakecode', // skip this if authorisation is not needed.
  ///   'password' : 'chocolate'
  /// };
  /// ```
  ///
  /// After initializing `client`, it attempts to connect to broker right away,
  /// and sets up streams for // TODO: finish this document.
  void init({
    @required Map config,
    @required String runtimeIdentifier,
  }) {
    client = MqttServerClient.withPort(
            config['server'], runtimeIdentifier, config['port'])
          ..autoReconnect = true
          ..logging(on: true)
          ..keepAlivePeriod = 4
    ...
    client.connectionMessage = MqttConnectMessage()
        .withClientIdentifier(runtimeIdentifier)
        .keepAliveFor(20)
        .authenticateAs(config['username'], config['password'])
        .withWillTopic('bakecode/runtime/will')
        .withWillMessage(runtimeIdentifier)
